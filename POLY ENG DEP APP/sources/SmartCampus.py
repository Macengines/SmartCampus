...
API_URL4 = "http://127.0.0.1:5000/api/notes"
...
class SignupScreen(MDBoxLayout):
    ...
    def store_details(self):
        ...
        cname = self.ids.cname.text.strip()
        level = self.ids.level.text.strip()
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO students (full_name, student_id, department, course_name, level)
                VALUES (%s, %s, %s, %s, %s)
            """, (fname, sid, dept, cname, level))
...
class HomeScreen(MDBoxLayout):
    ...
    def update_details(self, student):
        self.name = student.get("FULL NAME", "")
...
class FilesScreen(MDBoxLayout):
    ...
    def get_files(self, file_name, file_size, date_added, downloads, ratings):
        ...
        #rating formating
        try:
            formatted_rating = self.format_downloads(ratings)
        except ValueError as e:
            formatted_rating = ratings
